...
│   │   │   ├── login/
│   │   │   │   └── page.tsx     # ログインページ（Supabase Auth実装）【Step 3.2】
│   │   │   ├── auth/
│   │   │   │   └── callback/
│   │   │   │       └── route.ts # OAuth コールバック【Step 3.2】
│   │   │   ├── tilesets/
│   │   │   │   └── page.tsx     # タイルセット一覧
│   │   │   ├── features/
│   │   │   │   └── page.tsx     # フィーチャー一覧
...
│   │   │   ├── layout/
│   │   │   │   ├── index.ts
│   │   │   │   ├── sidebar.tsx   # サイドバー（ログアウト機能追加）【Step 3.2】
│   │   │   │   └── admin-layout.tsx # 管理画面レイアウト（ユーザー情報表示）【Step 3.2】
...
│   │   │       ├── badge.tsx
│   │   │       └── separator.tsx
│   │   ├── contexts/            # React Context【Step 3.2】
│   │   │   ├── index.ts
...
│   │   ├── hooks/               # カスタムフック（未実装）
│   │   ├── middleware.ts        # Next.js ミドルウェア（認証保護）【Step 3.2】
│   │   └── types/
│   │       └── index.ts         # 型定義
│   ├── public/                  # 静的ファイル
│   ├── .env.example             # 環境変数サンプル（Supabase追加）【Step 3.2】
│   ├── package.json             # Supabaseパッケージ追加【Step 3.2】
│   ├── package-lock.json
│   ├── tsconfig.json
│   ├── next.config.ts
│   ├── postcss.config.mjs
...
| ファイル | 役割 |
|---------|------|
| `/app/src/lib/supabase/client.ts` | ブラウザ用Supabaseクライアント |
| `/app/src/lib/supabase/server.ts` | サーバー用Supabaseクライアント |
| `/app/src/lib/supabase/middleware.ts` | セッション更新ユーティリティ |
| `/app/src/middleware.ts` | Next.js ミドルウェア（認証保護） |
| `/app/src/app/login/page.tsx` | ログインページ |
| `/app/src/app/auth/callback/route.ts` | OAuth コールバック |
| `/app/src/contexts/auth-context.tsx` | 認証コンテキスト |
...
| Admin UI Auth | @supabase/ssr | 0.5.x | **Step 3.2で追加** |
...
| 2025-12-13 | 0.9.0 | **Supabase Auth連携（Step 3.2完了）** - ログイン/ログアウト機能、認証ミドルウェア、認証コンテキスト |

---

*このドキュメントは2025-12-13時点の情報です。APIバージョン: 0.3.0 / MCPバージョン: 0.2.0 / Admin UIバージョン: 0.2.0*
