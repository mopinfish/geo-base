...
async def tool_search_features(
    ...
    Args:
        ...
        filter: Single property equality filter in format "key=value"
                (an optional "properties." prefix on the key is also
                accepted, e.g. "properties.type=station"). Only one
                equality condition is supported; the API rejects other
                syntax (comparisons, multiple conditions, etc.) with a
                400 error.
