...
class TestListTilesets:
    ...
    def test_list_tilesets_is_public_true_filters_exact(self):
        """list_tilesets(is_public=True) should return only public tilesets."""

        async def run_test():
            mock_response = Mock()
            mock_response.json.return_value = [
                {"id": "public", "name": "Public", "type": "vector", "is_public": True},
                {"id": "private", "name": "Private", "type": "vector", "is_public": False},
            ]
            mock_response.raise_for_status = Mock()
            ...
            with patch("tools.tilesets.httpx.AsyncClient") as mock_client:
                ...
                result = await list_tilesets(is_public=True)

                assert [ts["id"] for ts in result["tilesets"]] == ["public"]
                assert result["count"] == 1

        asyncio.run(run_test())

    def test_list_tilesets_is_public_false_filters_exact(self):
        """list_tilesets(is_public=False) should return only private tilesets."""
        ...
        async def run_test():
            ...
            with (
                patch("tools.tilesets.settings.api_token", "test-token"),
                patch("tools.tilesets.httpx.AsyncClient") as mock_client,
            ):
                mock_instance = AsyncMock()
                mock_instance.get.return_value = mock_response
                mock_instance.__aenter__.return_value = mock_instance
                ...
                result = await list_tilesets(is_public=False)

                assert [ts["id"] for ts in result["tilesets"]] == ["private"]
                assert result["count"] == 1

                mock_instance.get.assert_called_once()
                _, call_kwargs = mock_instance.get.call_args
                assert call_kwargs["params"] == {"include_private": "true"}

        asyncio.run(run_test())

    def test_list_tilesets_is_public_false_requires_auth(self):
        """list_tilesets(is_public=False) without an API token should error, not call the API."""

        async def run_test():
            with (
                patch("tools.tilesets.settings.api_token", None),
                patch("tools.tilesets.httpx.AsyncClient") as mock_client,
            ):
                mock_instance = AsyncMock()
                mock_instance.__aenter__.return_value = mock_instance
...
class TestSearchFeatures:
    ...
    def test_search_features_with_property_filter(self):
        """search_features should accept a property filter and forward it to the API request."""
        ...
        async def run_test():
            ...
            with patch("tools.features.httpx.AsyncClient") as mock_client:
                ...
                _, call_kwargs = mock_instance.get.call_args
                assert call_kwargs["params"]["filter"] == "type=station"
