...
async def analyze_area(
    ...
    with ToolCallLogger(
        ...
        try:
            ...
            logger.error(f"Unexpected error analyzing area: {e}")
...
async def calculate_distance(
    ...
    with ToolCallLogger(
        ...
        coords2_result = validate_coordinates(lat2, lng2)
        if not coords2_result.valid:
            result = create_error_response(
...
async def find_nearest_features(
    ...
    with ToolCallLogger(
        ...
        # Validate radius
        radius_result = validate_positive_number(radius_km, "radius_km")
        if not radius_result.valid:
            result = radius_result.to_error_response()
            log.set_result(result)
            return result

        # Validate limit
        limit_result = validate_limit(limit, max_value=100)
        ...
        try:
            ...
            # Apply limit
            features_with_distance = features_with_distance[:validated_limit]
            ...
            logger.error(f"Unexpected error finding nearest features: {e}")
            result = create_error_response(
                f"Unexpected error: {str(e)}",
                ErrorCode.UNKNOWN_ERROR,
                lat=lat, lng=lng,
...
async def get_buffer_zone_features(
    ...
    with ToolCallLogger(
        ...
        # Validate radii
        inner_result = validate_positive_number(inner_radius_km, "inner_radius_km", allow_zero=True)
        if not inner_result.valid:
            result = inner_result.to_error_response()
            log.set_result(result)
            return result

        outer_result = validate_positive_number(outer_radius_km, "outer_radius_km")
        if not outer_result.valid:
            result = outer_result.to_error_response()
            log.set_result(result)
            return result
        ...
        try:
            ...
            logger.error(f"Unexpected error getting buffer zone: {e}")
            result = create_error_response(
                f"Unexpected error: {str(e)}",
                ErrorCode.UNKNOWN_ERROR,
                lat=lat, lng=lng,
