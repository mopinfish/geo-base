...
async def list_tilesets(
    ...
    Args:
        ...
        is_public: Filter by public/private status:
            - None (default): use the API's default result set (public tilesets)
            - True: return only public tilesets
            - False: return only your own private tilesets (requires API_TOKEN)
    ...
    with ToolCallLogger(logger, "list_tilesets", type=type, is_public=is_public) as log:
        ...
        # Filtering to private-only tilesets requires authentication; fail fast
        # rather than issuing a request that the API will reject.
        if is_public is False and not settings.api_token:
            result = create_error_response(
                "Authentication required to list private tilesets",
                ErrorCode.AUTH_REQUIRED,
                hint="Configure API_TOKEN in environment to access private tilesets.",
                tilesets=[],
                count=0,
            )
            log.set_result(result)
        ...
        try:
            ...
            # The API's is_public/include_private handling is best-effort, so
            # enforce the exact boolean filter here regardless of what the API
            # returned.
            if is_public is not None:
                normalized_tilesets = [
                    ts for ts in normalized_tilesets if ts.get("is_public", True) == is_public
                ]

            # Add summary
            result = {
                "tilesets": normalized_tilesets,
                "count": len(normalized_tilesets),
