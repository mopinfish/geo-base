...
並行性テスト（100 並行リクエスト）で Redis backend の正確性を確認済み（`api/tests/test_auth/test_api_key_rate_limit.py::TestConcurrency`）。本番反映時の手順とフォールバック挙動は `docs/AUTH_SETUP.md` § API キー rate limit 参照。
