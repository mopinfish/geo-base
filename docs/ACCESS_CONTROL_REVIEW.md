...
⚠️ **`api/lib/routers/features.py`** および **`api/lib/routers/datasources.py`** はまだ旧版の `check_tileset_access()` を使用しており、**チーム共有を考慮していない**。
...
#### I-1. メンバーがチームタイルセットを「追加」できるが「削除」できない非対称

`api/lib/routers/teams.py:892` (add) は member も許可しているが、`953` (delete) は owner/admin のみ。誤って追加したものを当人が取り消せない。

**対応:** どちらかに統一。member の追加権限を絞る方向が安全。
...
| 🟡 Important | I-1 メンバー追加/削除非対称 | 仕様確定の上、対応 |
