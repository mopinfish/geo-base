...
> English: [ACCESS_CONTROL_REVIEW.md](./ACCESS_CONTROL_REVIEW.md) ・ 日本語: this page
...
> 関連ドキュメント:
> - 認証本体: [`AUTH_SETUP.ja.md`](./AUTH_SETUP.ja.md)
...
#### I-3. レート制限が DB 同期で書き込み競合を起こしうる ✅ 対応済み

`api_key_rate_limits` を DB INSERT で集計するため、高頻度アクセスで lock contention が発生していた。本番想定のトラフィックでは Redis または in-memory アグリゲーション + 定期 flush が必要。

**対応 (Issue #56 / PR #121):** `RATE_LIMIT_BACKEND=redis` で Redis INCR + EXPIRE による fixed-window 集計に切り替え可能。`RateLimiter` protocol を導入して `DbRateLimiter` / `RedisRateLimiter` の 2 実装を持つ構成にし、env で動的に切替可能（既定は後方互換のため `db`）。Redis 障害時は **fail-open**（warn ログ + リクエスト通過）で可用性を優先。

並行性テスト（100 並行リクエスト）で Redis backend の正確性を確認済み（`api/tests/test_auth/test_api_key_rate_limit.py::TestConcurrency`）。本番反映時の手順とフォールバック挙動は `docs/AUTH_SETUP.ja.md` § API キー rate limit 参照。

なお `api_key_rate_limits` テーブルは Redis backend 採用後も残してあり、`RATE_LIMIT_BACKEND=db` で即時ロールバック可能。テーブル削除は Redis backend が本番で安定稼働してから別 Issue で実施する。
...
| 🟡 Important | I-3 レート制限 Redis 化 | ✅ 対応済み（Issue #56 / PR #121。`RATE_LIMIT_BACKEND=redis` で切替、fail-open 設計） |
