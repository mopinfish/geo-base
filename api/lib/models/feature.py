...
class BulkFeatureCreate(BaseModel):
    ...
    update_bounds: bool = Field(
        True, description="Automatically update tileset bounds after import"
    )
    validate_geometry: bool = Field(True, description="Validate geometry structure before import")
...
class BulkFeatureResponse(BaseModel):
    ...
    bounds: Optional[List[float]] = Field(
        None, description="Updated bounds [west, south, east, north]"
    )
    center: Optional[List[float]] = Field(None, description="Updated center [longitude, latitude]")
