...
def parse_feature_property_filter(filter_str: str) -> tuple[str, list[str]]:
    """
    Parse a feature property equality filter into SQL fragment and parameters.

    Accepts simple format: "key=value" or "properties.key=value"
    Returns SQL fragment with positional placeholders and parameter list.

    Args:
        filter_str: Filter expression (e.g., "type=station" or "properties.type=station")

    Returns:
        Tuple of (SQL WHERE fragment, [key, value]) where SQL fragment uses %s
        placeholders for parameter binding

    Raises:
        ValueError: If filter_str is invalid (empty, malformed, contains reserved
                   characters, etc.)
    """
    # Reject if empty
    if not filter_str:
    ...
    # Reject if contains semicolon or comma anywhere
    if ";" in filter_str or "," in filter_str:
        raise ValueError("Filter expression cannot contain ';' or ','")

    # Split on first equals sign
    key, sep, value = filter_str.partition("=")
    key = key.strip()

    # Reject if no separator found
    if not sep:
        raise ValueError("Filter expression must contain '='")

    # Reject if empty key or value
    if not key or not value:
        raise ValueError("Key and value cannot be empty")

    # Check for invalid characters in the key (before stripping prefix)
    if any(c in key for c in "!><~"):
        raise ValueError("Key cannot contain '!', '>', '<', or '~'")

    # Remove leading "properties." prefix if present
    if key.startswith("properties."):
        key = key[len("properties.") :]

    # After stripping prefix, require key to be non-empty
    if not key:
        raise ValueError("Key cannot be empty after removing 'properties.' prefix")

    # Return SQL fragment and parameters list (both key and value are parameters)
    return "f.properties->>%s = %s", [key, value]
