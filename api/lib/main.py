...
from fastapi import Depends, FastAPI, HTTPException, Query, Request, Response, Body, UploadFile, File
from fastapi.middleware.cors import CORSMiddleware
from fastapi.responses import HTMLResponse
from pydantic import BaseModel, Field

# lib.raster_tilesのimportを更新
    ...
    <script>
        ...
        fetch('/api/health/db')
            ...
            .then(data => {
                ...
                el.textContent = data.status === 'ok' ? 'OK' : 'NG' + data.database;
                el.className = 'status ' + (data.status === 'ok' ? 'ok' : 'error');
            })
            .catch(() => {
                const el = document.getElementById('db-status');
                el.textContent = 'DATABASE CONNECTION ERROR';
