...
def create_api_key(
    key_data: ApiKeyCreate, conn=Depends(get_connection), user: User = Depends(require_auth)
):
    """
...
def list_api_keys(
    ...
    try:
        ...
        with conn.cursor() as cur:
            # Get total count
            cur.execute(f"SELECT COUNT(*) FROM api_keys ak WHERE {where_clause}", tuple(params))
...
def get_api_key(key_id: str, conn=Depends(get_connection), user: User = Depends(require_auth)):
...
def delete_api_key(key_id: str, conn=Depends(get_connection), user: User = Depends(require_auth)):
...
def get_api_key_usage(
    ...
    try:
        ...
        with conn.cursor() as cur:
            cur.execute("SELECT * FROM get_api_key_usage_stats(%s, %s)", (key_id, days))
            row = cur.fetchone()
...
def get_api_key_logs(
    ...
    try:
        ...
        with conn.cursor() as cur:
            # Get total count
            cur.execute("SELECT COUNT(*) FROM api_key_usage_logs WHERE api_key_id = %s", (key_id,))
            total = cur.fetchone()[0]
        ...
        return ApiKeyUsageLogResponse(logs=logs, total=total, key_id=key_id)
...
def get_rate_limit_status(
    key_id: str, conn=Depends(get_connection), user: User = Depends(require_auth)
):
    ...
    try:
        ...
        with conn.cursor() as cur:
            # Get minute status
            cur.execute("SELECT * FROM get_api_key_rate_limit_status(%s, 'minute')", (key_id,))
            minute_row = cur.fetchone()

            # Get day status
            cur.execute("SELECT * FROM get_api_key_rate_limit_status(%s, 'day')", (key_id,))
...
def validate_api_key_endpoint(request: Request, conn=Depends(get_connection)):
    """
    ...
    try:
        ...
        if not api_key:
            return JSONResponse(
                status_code=401, content={"valid": False, "error": "No API key provided"}
            )
        ...
        if not row:
            return JSONResponse(
                status_code=401, content={"valid": False, "error": "Invalid or expired API key"}
            )
