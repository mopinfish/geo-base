...
def make_tileset_with_feature(db_conn):
    ...
    def _make(owner_id=None, is_public=False, properties=None):
            ...
            cur.execute(
                """INSERT INTO features (id, tileset_id, layer_name, geom, properties)
                   VALUES (%s, %s, 'default',
                           ...
                           %s::jsonb)""",
                (feature_id, ts_id, properties_json),
            )
...
class TestFeaturesPropertyFilter:
    """GET /api/features?filter=key=value."""

    def test_filter_matches_ascii_and_japanese_property(
        self,
        client_for,
        make_tileset_with_feature,
    ):
        token = uuid.uuid4().hex
        type_value = f"station-{token}"
        quake_value = f"◎-{token}"
        make_tileset_with_feature(
            is_public=True,
            properties={"type": type_value, "地震": quake_value},
        )
        client = client_for(None)

        response = client.get("/api/features", params={"filter": f"type={type_value}"})
        assert response.status_code == 200, response.text
        assert response.json()["total_count"] == 1
        assert len(response.json()["features"]) == 1

        japanese = client.get("/api/features", params={"filter": f"地震={quake_value}"})
        assert japanese.status_code == 200, japanese.text
        assert japanese.json()["total_count"] == 1

    def test_filter_no_match_returns_zero_count(
        self,
        client_for,
        make_tileset_with_feature,
    ):
        token = uuid.uuid4().hex
        type_value = f"station-{token}"
        make_tileset_with_feature(is_public=True, properties={"type": type_value})
        client = client_for(None)

        other_token = uuid.uuid4().hex
        response = client.get("/api/features", params={"filter": f"type=nonexistent-{other_token}"})
        assert response.status_code == 200, response.text
        body = response.json()
        assert body["total_count"] == 0
        assert body["features"] == []

    def test_filter_composed_with_layer(
        self,
        client_for,
        make_tileset_with_feature,
    ):
        ...
        response = client.get(
            "/api/features",
            params={"filter": f"type={type_value}", "layer": "default"},
        )
        assert response.status_code == 200, response.text
        assert response.json()["total_count"] == 1
    ...
    @pytest.mark.parametrize(
        "invalid_filter",
        ["type!=station", "type=station;name=x", "bad>value"],
    )
    def test_filter_invalid_syntax_returns_400(
        self,
        client_for,
        make_tileset_with_feature,
        invalid_filter,
    ):
        make_tileset_with_feature(is_public=True)
        client = client_for(None)

        response = client.get("/api/features", params={"filter": invalid_filter})
        assert response.status_code == 400, response.text

    def test_filter_excludes_private_tileset_in_cross_tileset_query(
        self,
        client_for,
        make_tileset_with_feature,
    ):
        """Cross-tileset filter search (no tileset_id) must still exclude
        private tilesets, even when their properties match the filter."""
        token = uuid.uuid4().hex
        type_value = f"station-{token}"
        make_tileset_with_feature(is_public=False, properties={"type": type_value})
        client = client_for(None)
